...
def Mean_Shift(sourcePath="input.csv", outputPath="output.csv", quantile=0.3):
    ...
    for cluster_id in labels_unique:
        ...
        if (CLUSTER_REPRESENTATIVE_METHOD == 0):
            ...
            clusters_representative[cluster_id] = [max(0, center_of_cluster_id[0] - mean_width / 2),
                                                   max(0, center_of_cluster_id[1] - mean_height / 2), mean_width,
                                                   mean_height, mean_score]
        else:
    ...
    # Exporting CSV file
    clusters_representative_with_rowcol = np.array(
        [[int(e[0]), int(e[1]), int(e[2]), int(e[3]), e[4]] for e in clusters_representative])
    clusters_representative_with_rowcol = DataFrame(clusters_representative_with_rowcol, columns=RECTANGLES)
    clusters_representative_with_rowcol.to_csv(path_or_buf=outputPath, columns=RECTANGLES, encoding='utf-8')
...
def windowMetric(rectangle_1, rectangle_2):
    ...
    """
    intersection_h = min(rectangle_1[2], rectangle_2[2], rectangle_1[0] + rectangle_1[2] - rectangle_2[0],
                         rectangle_2[0] + rectangle_2[2] - rectangle_1[0])
    intersection_w = min(rectangle_1[3], rectangle_2[3], rectangle_1[1] + rectangle_1[3] - rectangle_2[1],
                         rectangle_2[1] + rectangle_2[3] - rectangle_1[1])
...
def RDBSCAN(sourcePath="input.csv", outputPath="output.csv", quantile=0.3):
    ...
    clusters_representative_output.to_csv(path_or_buf=outputPath, encoding='utf-8')

    print(clusters_representative)


def clustering(choice, sourcePath, outputPath, quantile=0.3):
    ...
    if int(choice) == 0:
        ...
        Mean_Shift(sourcePath, outputPath, quantile)
		
    else:
		
        RDBSCAN(sourcePath, outputPath, quantile)
